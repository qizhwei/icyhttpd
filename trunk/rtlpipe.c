...
int RtlCreatePipe(HANDLE *pipeRead, HANDLE *pipeWrite)
{
	char buffer[48];
	HANDLE pipe0, pipe1;
	...
	pipe0 = CreateNamedPipeA(buffer, FILE_FLAG_OVERLAPPED | PIPE_ACCESS_INBOUND,
	...
	pipe1 = CreateFileA(buffer, GENERIC_WRITE, 0, NULL,
...
int RtlCreatePipeDuplex(HANDLE *pipeServer, HANDLE *pipeClient)
{
	char buffer[48];
	HANDLE pipe0, pipe1;
