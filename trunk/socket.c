...
MAYFAIL(-1) int socket_init(void)
{
	WSADATA data;
...
static inline NOFAIL socket_t *socket_create_from(SOCKET os_socket)
{
...
MAYFAIL(NULL) socket_t *socket_create(void)
{
	SOCKET os_socket;
	...
	return socket_create_from(os_socket);
}
...
MAYFAIL(-1) int socket_bind_ip(socket_t *s, const char *ip, int port)
{
	ip_endpoint_t ep;
	HANDLE event;
...
MAYFAIL(NULL) socket_t *socket_accept(socket_t *s)
{
...
MAYFAIL(-1) ssize_t socket_read(socket_t *s, void *buffer, size_t size)
{
...
MAYFAIL(-1) ssize_t socket_write(socket_t *s, void *buffer, size_t size)
{
	WSABUF buf = {size, buffer};
	async_io_t async;
	size_t result;
